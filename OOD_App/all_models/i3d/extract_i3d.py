...
import torch
import torchvision
from ...all_models._base.base_extractor import BaseExtractor
from ...all_models.i3d.i3d_src.i3d_net import I3D
from ...all_models.pwc.extract_pwc import DATASET_to_PWC_CKPT_PATHS
from ...all_models.raft.extract_raft import DATASET_to_RAFT_CKPT_PATHS
from ...all_models.raft.raft_src.raft import RAFT, InputPadder
from ...all_models.transforms import (Clamp, PermuteAndUnsqueeze, PILToTensor,
...
class ExtractI3D(BaseExtractor):
    ...
    def load_model(self) -> Dict[str, torch.nn.Module]:
        ...
        i3d_weights_paths = {
            'rgb': f'{os.getcwd()}/OOD_App/all_models/i3d/checkpoints/i3d_rgb.pt',
            'flow': f'{os.getcwd()}/OOD_App/all_models/i3d/checkpoints/i3d_flow.pt',
        }
        name2module = {}           
        ...
        if "flow" in self.streams:
            ...
            if self.flow_type == 'pwc':
                ...
                print("pwc")
                from ...all_models.pwc.pwc_src.pwc_net import PWCNet
