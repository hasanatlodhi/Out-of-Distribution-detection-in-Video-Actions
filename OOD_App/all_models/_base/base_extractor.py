...
class BaseExtractor(object):
    ...
    def _extract(self, video_path: str):
        ...
        try:
            ...
            if not self.is_already_exist(video_path):
                ...
                concatenated_features=np.concatenate((feats_dict['rgb'],feats_dict['flow']),axis=1)
    ...
    def action_on_extraction(self,feats_dict: Dict[str, np.ndarray],video_path: str,) -> None:
        ...
        if self.on_extraction in ['save_numpy', 'save_pickle'] and self.is_already_exist(video_path):
            ...
            return
        final_arr=np.concatenate((feats_dict['rgb'],feats_dict['flow']),axis=1)
        final_labels=np.array([video_path]*len(feats_dict['rgb']))
        print("\n\nAnddddd Gottttt:  ",final_arr)
        try:
            feat=np.load('one_video.npy')
            labels=np.load('one_labels.npy')
            final_arr=np.concatenate((feat,final_arr),axis=0)
            final_labels=np.append(labels,final_labels)
        except:
            print("File not available")
        np.save('one_video.npy',final_arr)
        np.save('one_labels.npy',final_labels)
